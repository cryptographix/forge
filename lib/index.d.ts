...
declare module "node-forge" {
    type Byte = number;
    type Bytes = string; // | ArrayBuffer | ArrayBufferView | ByteStringBuffer;
    type Hex = string;
    type Base64 = string;
    type Utf8 = string;
    type OID = string;
    ...
    namespace jsbn {
        ...
        class BigInteger {
            ...
            // Sort of internals - see @types/jsbn
    ...
    namespace cipher {
        ...
        type PadFunc = (data:util.ByteBuffer,opts:{decrypt:boolean, overflow:number})=>boolean;

        interface BlockCipher {
            start: (options?: StartOptions) => void;
            update: (payload: util.ByteBuffer) => void;
            finish: (pad?:boolean|PadFunc) => boolean;
    ...
    namespace pss {
        type PSS = {
          encode(md: md.MessageDigest, modBits:number): Bytes;
          verify(mHash: Bytes, em: Bytes, modBits:number): boolean;
        };

        type PSSParams = {
          md: md.MessageDigest;
          mgf?: { generate( h, maskLen ): any; },
          saltLength?: number;
          salt?: string;
          prng?: any;
        };

        function create(params: PSSParams): PSS;
    }
